...
DataExtractor::DataExtractor(std::string name) // constructor
{
		...
		dataArray = ArrayAllocator::AllocateDynamicArray<double>(number_of_lines,3); //allocates an array of the appropriate size, right now 3 wide MAY NEED ADJUSTMENT
...
{		
	...
	for(int i=0; i < (int) (*stringDat).size(); i++) // for each line in the buffer (each data file line).
	{
		...
		for(int j=0; j < 3; j++) //for x and y
		{ 
			
			numDat[i][j] =  atof(vbuff1[j].c_str()); // convert string to float	and stores it
